    ...
    <>
      {isLoading ? (
        ...
        <>
          ...
          </h1> <p className="text-center">Select a company to view ownership</p> 
          <>
          {lightMode ? (
          ...
          </DropdownButton><><p className="text-right">Total worth of <b>{associated_company_id}</b> is <b>$ {totals}</b></p></>
          ...
          {sumResult.length === 0 ? (
            <>
              ...
              <p className="text-center">
                If you would like to see company ownership of this company, please create a transaction for the company
              </p>
            ...
            <VictoryPie  style={{
              data: {
                fillOpacity: 0.9, stroke: "#03dac5", strokeWidth: 2
              },
              labels: {
                fontSize: 10.5, fill: "#03dac5"
              }
            }} data={sumResult} x="user" y="amount"  theme={VictoryTheme.material} colorScale="blue"  height={250} width={450}animate={{
              duration: 2000,
              easing: "exp"
            }} endAngle={endAngle} events={[{
              target: "data",
              eventHandlers: {
                onClick: () => {
                  return [
                    {
                      ...
                      target: "labels",
                      mutation: ({ datum, text }) => {
                        return  text === `${datum.user} ${Math.round((datum.amount/totals)*100)}%` ? null : {text: `${datum.user} ${Math.round((datum.amount/totals)*100)}%`}
                      }
                    }
                  ];
                }
